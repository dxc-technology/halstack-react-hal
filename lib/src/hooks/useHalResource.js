  ...
  useEffect(() => {
    ...
    const getInteractions = halResource => {
      return halResource
        ...
        .map(interaction => ({
          rel: interaction.rel,
          handler: getInteractionHandler(interaction.method, halResource.getLink("self")),
        }))
        .reduce(
          (obj, interaction) => ({
            ...obj,
    ...
    const getInteractionHandler = (method, resourceSelf) => {
        ...
        return HalApiCaller[apiCallerFunctionName]({
          ...
          .then((response) => {
            if (
              ...
              response.halResource.getLink("self") === resourceSelf.href
            ) {
              setStatus("resolved");
              setResource(response.halResource);
            } else {
